...
class HandDataset(utils.Dataset):
    ...
    def load_hand(self, dataset_dir, subset):
        ...
        image_ids = [f for f in os.listdir(image_dir) if os.path.isfile(os.path.join(image_dir, f))]
    ...
    def load_mask(self, image_id):
        ...
        mask_dir_files = [f for f in os.listdir(mask_dir) if os.path.isfile(os.path.join(mask_dir, f))]
        file_base =  os.path.basename(info['path']).split('.')[0]
        mask_files = [mask_file for mask_file in mask_dir_files if mask_file.startswith(file_base)]

        mask = []
        for mask_file in mask_files:
           mask_channel = skimage.io.imread(os.path.join(mask_dir, mask_file)).astype(np.bool)
